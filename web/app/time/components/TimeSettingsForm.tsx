...
import { getDefaultWorkingDaysConfig } from '@/lib/calc/dates'
...
interface TimeSettingsFormProps {
  ...
  clinicId?: string
  onRefreshWorkingDays?: () => Promise<void>
}

export function TimeSettingsForm({ form, clinicId, onRefreshWorkingDays }: TimeSettingsFormProps) {
  const t = useTranslations('time')
  const v = useTranslations('validation')

  // Get working_days_config from form or use default
  const workingDaysConfig = form.watch('working_days_config') || getDefaultWorkingDaysConfig()

  const handleWorkingDaysChange = (config: WorkingDaysConfig) => {
    form.setValue('working_days_config', config, { shouldValidate: true })
  }
    ...
    <Form {...form}>
        ...
        <FormGrid columns={1}>
          <InputField
            ...
            onChange={(e) => {
              ...
              form.setValue('work_days', Number(value) || 0)
            }}
          ...
          <InputField
            ...
            onChange={(e) => {
              const value = typeof e === 'object' ? e.target.value : e
              form.setValue('hours_per_day', Number(value) || 0)
            }}
          ...
          <InputField
            ...
            onChange={(e) => {
              const value = typeof e === 'object' ? e.target.value : e
              form.setValue('real_pct', Number(value) || 0)
            }}
      ...
      <div className="mt-6">
        <WorkingDaysSelector
          ...
          clinicId={clinicId}
          onRefreshPattern={onRefreshWorkingDays}
        />
      </div>
    </Form>
  )
