/**
 * API Endpoint: Optimize Inventory Action
 *
 * Analyzes supply usage and recommends reorder points.
...
const optimizeInventorySchema = z.object({
  ...
  reorder_threshold_pct: z.coerce.number().min(0).optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = validateSchema(optimizeInventorySchema, bodyResult.data)
    if ('error' in parsed) {
      return parsed.error
    }
    ...
    const result = await aiService.execute('optimize_inventory', params, {
      clinicId: clinic_id,
      userId: user.id,
    ...
    console.error('[API] /api/actions/optimize-inventory error:', error)
    return NextResponse.json(
      {
        success: false,
        error: {
