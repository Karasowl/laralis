-- =================================================================
-- SCRIPT ÚNICO Y COMPLETO: Fix de Onboarding
-- =================================================================
...
BEGIN
    ...
    INSERT INTO diagnostico_final VALUES ('1. category_types inicial', '📊', v_count || ' registros encontrados');
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO diagnostico_final VALUES ('1. UNIQUE constraint', '✅', 'Agregado a category_types.name');
    ELSE
        INSERT INTO diagnostico_final VALUES ('1. UNIQUE constraint', 'ℹ️', 'Ya existe');
    END IF;
    ...
    -- Verificar de nuevo
    SELECT COUNT(*) INTO v_count FROM category_types;
    INSERT INTO diagnostico_final VALUES ('1. category_types final', '✅', v_count || ' tipos disponibles');
END $$;
...
INSERT INTO diagnostico_final VALUES ('2. RLS', '✅', 'Habilitado en 3 tablas');
...
INSERT INTO diagnostico_final VALUES ('3. Políticas RLS', '✅', '9 políticas creadas');
...
INSERT INTO diagnostico_final VALUES ('4. Funciones trigger', '✅', 'Modificadas a SECURITY DEFINER');
...
BEGIN
    ...
    INSERT INTO diagnostico_final VALUES ('5. Verificación tipos',
        CASE WHEN v_cat_types >= 4 THEN '✅' ELSE '❌' END,
        v_cat_types || ' tipos en category_types');

    INSERT INTO diagnostico_final VALUES ('5. Verificación políticas',
        CASE WHEN v_policies >= 9 THEN '✅' ELSE '❌' END,
        v_policies || ' políticas RLS activas');

    INSERT INTO diagnostico_final VALUES ('5. Verificación SECURITY DEFINER',
...
-- =================================================================
-- RESULTADO FINAL - TODO EN UNA SOLA CONSULTA
-- =================================================================
SELECT
    '========================================' as " ",
    '🎉 DIAGNÓSTICO Y FIX COMPLETO' as "RESULTADO"
UNION ALL
SELECT '========================================', ''
UNION ALL
SELECT paso, estado || ' ' || detalle FROM diagnostico_final ORDER BY paso
UNION ALL
SELECT '========================================', ''
UNION ALL
