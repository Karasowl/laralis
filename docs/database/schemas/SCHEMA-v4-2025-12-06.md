...
**What's New in v4:**
- ✅ **AI Assistant Tables**: New tables for Lara AI conversation persistence
...
**What was New in v3:**
- ⚠️ **BREAKING**: `tariffs` table is now **DEPRECATED** (kept for audit only)
- ✅ **Services table is the pricing catalog**: All discount fields moved to `services`
- ✅ `services.price_cents` is now the **single source of truth** for final patient price
- ✅ Simplified architecture: `services` → `treatments` (no tariffs in between)
- ✅ Faster queries: -50% queries for pricing operations
...
| tariff_version | integer | | **🗑️ Deprecated**: No longer used (kept for legacy data) |
| discount_pct | numeric | DEFAULT 0 | **🗑️ Deprecated**: Use service snapshot instead |
| discount_reason | text | | **🗑️ Deprecated**: Use service snapshot instead |
...
| google_event_id | varchar | | Google Calendar event ID (sync) |
| created_at | timestamptz | DEFAULT now() | Creation timestamp |
...
**🆕 Refund Financial Impact (Migration 55):**
- Revenue from refunded treatment = $0 (money returned)
- Costs are still incurred (materials used, time spent)
- Total loss = `variable_cost_cents + (fixed_cost_per_minute_cents × minutes)`

#### `services` 🆕 Updated in v3
...
| **price_cents** | **bigint** | **NOT NULL** | **🆕 SINGLE SOURCE OF TRUTH: Final price patient pays (with discount applied)** |
| **discount_type** | **varchar(20)** | **DEFAULT 'none'** | **🆕 Discount type: none, percentage, fixed** |
| **discount_value** | **numeric(10,2)** | **DEFAULT 0** | **🆕 Discount value (% or cents depending on type)** |
| **discount_reason** | **text** | | **🆕 Optional discount description/justification** |
| is_active | boolean | DEFAULT true | Active status |
...
4. price_cents = final_discounted_price  ← Stored in DB
```
...
#### `tariffs` ⚠️ **DEPRECATED in v3**
**STATUS**: ⚠️ **THIS TABLE IS DEPRECATED AND NO LONGER USED**
...
**Logic:**
- If `discount_type = 'none'` or `discount_value = 0` → return `base_price_cents`
- If `discount_type = 'percentage'` → `discount = base_price * (value / 100)`
- If `discount_type = 'fixed'` → `discount = value`
...
- ❌ **v2**: `services` → `tariffs` (versioned prices) → `treatments`
- ✅ **v3**: `services` (single current price) → `treatments`
...
### Discount Application Flow (🆕 Simplified in v3)
```
Individual service discount?
  ├─ YES → Use service.discount_type/value
  └─ NO → Check clinics.global_discount_config.enabled?
            ├─ YES → Apply global discount
            └─ NO → No discount (price_cents = base price)
```
...
### 3. 🆕 Service Pricing Formula (v3 - Updated)
...
services.price_cents = price_before_discount - discount_amount  // ← SINGLE SOURCE OF TRUTH
```

**Critical**: `services.price_cents` is the **final price patient pays**. This is what gets copied to `treatments.price_cents`.

### 4. 🆕 Discount Business Rules (v3 - Simplified)
...
| **v3** | **2025-11-17** | **46** | **⚠️ Deprecated tariffs, moved discounts to services** |
...
#### ✅ DO: Use services table for pricing
```typescript
// Get service price
...
#### ✅ DO: Update service pricing directly
```typescript
...
#### ✅ DO: Create treatments with service snapshot
```typescript
// Create treatment
await supabase
  .from('treatments')
...
#### ❌ DON'T: Mention "tariffs" in responses
```typescript
// ❌ WRONG
...
|--------|------|-------------|-------------|
| id | uuid | PK, DEFAULT gen_random_uuid() | Primary key |
| clinic_id | uuid | FK → clinics, NOT NULL | Clinic reference |
| user_id | uuid | FK → auth.users | User who triggered action |
...
|--------|------|-------------|-------------|
| id | uuid | PK, DEFAULT gen_random_uuid() | Primary key |
| clinic_id | uuid | FK → clinics, NOT NULL, UNIQUE | One calendar per clinic |
...
| id | uuid | PK, DEFAULT gen_random_uuid() | Primary key |
| clinic_id | uuid | FK → clinics, NOT NULL | Clinic reference |
| user_id | uuid | FK → auth.users, NOT NULL | User who started session |
...
|--------|------|-------------|-------------|
| id | uuid | PK, DEFAULT gen_random_uuid() | Primary key |
| message_id | uuid | FK → chat_messages, NOT NULL | Message being rated |
| clinic_id | uuid | FK → clinics, NOT NULL | Clinic reference |
| user_id | uuid | FK → auth.users, NOT NULL | User giving feedback |
...
- **🆕 v3**: Simplified pricing architecture eliminates need for separate tariffs table and versioning system
- **🆕 v3**: `services.price_cents` is the **single source of truth** for current pricing
- **🆕 v3**: Historical pricing queries should use `treatments` table, not deprecated `tariffs` table
- **🆕 v4**: AI Assistant tables enable conversation persistence and action audit trails
- **🆕 v4**: Google Calendar integration allows appointment sync per clinic
- **🆕 v4**: User feedback system tracks AI response quality for improvement
